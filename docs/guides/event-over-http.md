...
each other without a loop. It is engine-internal: the worker removes it (together with any
transported `my_*` key) from a function's input header copy *and* from the delivered
envelope's own header view, and filters it from returned response headers, so application
code never sees or emits it.
