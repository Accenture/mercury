...
| `traceparent` | W3C Trace Context: `00-{trace-id}-{parent-span-id}-01` (32-hex trace id, 16-hex span id). Inbound it **takes precedence** over `X-Trace-Id` and contributes the caller's span as this hop's parent; outbound it is emitted alongside `X-Trace-Id`, built from this hop's own span. The carrier **name** is configurable via `http.traceparent.header` (default `traceparent`; per-endpoint `traceparent.header` in rest.yaml) — for **backward compatibility with legacy systems only**, e.g. an intermediary that strips the standard header: the same W3C value then travels under **both** names. Inbound, the standard `traceparent` always wins; the custom name is read only when the standard is absent (its presence means the caller already upgraded to the standard — a residual proprietary value is safely ignored). Departure from the W3C/OpenTelemetry standard is discouraged — treat a custom name as a temporary bridge (see [Observability](observability.md#two-ids-two-concerns)) |
...
| `x-ttl` | Time-to-live in **milliseconds** — the one representation of a request's time budget (`AsyncHttpRequest::set_timeout_seconds` writes it; default 30 seconds when absent on a client call). REST automation stamps the **endpoint timeout** under this key on the request dataset at ingress (a caller-sent value wins), so a flow's `input.header` view carries it on both engines |
