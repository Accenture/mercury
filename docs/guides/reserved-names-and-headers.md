...
| `traceparent` | W3C Trace Context: `00-{trace-id}-{parent-span-id}-01` (32-hex trace id, 16-hex span id). Inbound it **takes precedence** over `X-Trace-Id` and contributes the caller's span as this hop's parent; outbound it is emitted alongside `X-Trace-Id`, built from this hop's own span. The carrier **name** is configurable via `http.traceparent.header` (default `traceparent`; per-endpoint `traceparent.header` in rest.yaml) — an escape hatch for an intermediary that strips the standard header: the same W3C value then travels under **both** names, and a well-formed value under the custom name wins inbound (see [Observability](observability.md#two-ids-two-concerns)) |
