...
**Rapid prototyping — deploy without a rebuild.** The manifest is an ordinary property, so a `-D`
runtime override (a program argument in the Rust port) sets it for one run, and `location` accepts a
`file:/` folder. Stage the export in a folder with its own manifest and restart with the override —
