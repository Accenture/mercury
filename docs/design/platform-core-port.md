...
| D4 | **Idiomatic serde wire format** (not byte-compatible with Java) | The Kafka mesh / cross-JVM distribution is out of scope, so Rust↔Java envelope interop isn't needed. Revisit only if interop is ever required. **Revisited 2026-07-21 (increment 59, Event over HTTP phase 2): interop IS now required — resolved by Java adopting a named-key "standard" wire format that matches this port's `to_vec_named` output.** The normative spec lives in the Java repo (`docs/guides/event-envelope-wire-format.md`); shared golden vectors under `tests/resources/envelope-vectors/` prove conformance both ways. |
