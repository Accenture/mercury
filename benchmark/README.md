...
java -jar target/benchmark-client-2.8.3.jar
```
...
java -jar target/benchmark-server-2.8.3.jar
cd sandbox/mercury/connectors/adapter/kafka/kafka-presence
java -jar target/kafka-presence-2.8.3.jar
cd sandbox/mercury/benchmark-client
java -Dcloud.connector=kafka -jar target/benchmark-client-2.8.3.jar
```
...
The number of workers per service is configured to be 50. When the number of concurrent requests
is more than 50, the system will queue the outstanding requests orderly.
