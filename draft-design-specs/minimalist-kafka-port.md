...
| K5b ✅ | **The held items close**: sync-over-async facade tasks (`sync.prepare`/`sync.await`/`soa.reply`) over this transport + the demo mirrored from Java (facade/backend roles, the raw, JSON Schema and Avro legs), driven live against the Java helpers and the Java demo | **DONE 2026-09-21** — `mercury-sync-over-async` gains `tasks` (the three facade functions, transport-neutral, 250 workers each, the Java contracts: `begin` before the publish, `await_response` with the `x-sync-timeout` budget → 408 `Timed out awaiting response for <cid>`, `deliver` → `{cid, delivered}`) and `bootstrap` (the Java `SyncOverAsyncAutoStart`: the coordinator from `sync.over.async.enabled` + `soa.redis.*`, the health probe on every pod, sequence 15 before the Kafka adapter's 20); the MVP e2e `rest_flow_mvp.rs` (REST → flow → MockCluster → flow → RESP double → 200 with the echo and a continuous trace id; 408 on `no-reply`) is the Java `RestFlowMvpTest` verbatim in resources. `examples/sync-over-async-demo` mirrors the Java demo one to one: the nine flows, both adapter YAMLs, `rest.yaml`, the `facade`/`backend` profiles, the registry seeds and Node helpers, `soa.rs` (the raw / JSON / Avro backend variants, the two decoded-reply variants, the status policy) with the `SoaTaskTest` twins. Live drive (report addendum, Scenarios 8–13): all three legs on Rust alone with continuous trace ids; 408 in 3 s; Java backend + Rust facade and Java facade + Rust backend, the Confluent frames decoded in both directions by both engines; a mixed backend group (20 requests, 9 Rust / 11 Java); the Java facade delivering 8 of 20 replies to the waiting Rust facade through the Redis return route — cross-pod AND cross-engine; two Rust facades. Finding 5: the link-line caveat (§7 item 6) applies to every configuration-only crate — the demo shipped without `http.flow.adapter` until two `use … as _;` lines were added |
| K5c | **The release gate**: the guide twin for the AI contract (`docs/guides/minimalist-kafka.md`, branch `docs/minimalist-kafka-guide` — DONE 2026-09-21 with the nav, `llms.txt`, `files.list`, the configuration reference's Kafka section, two registered claims, the README non-goals and port-scope pages), one INCREMENTS entry for the whole port (Increment 127); then v4.12.14 on both engines publishes `mercury-sync-over-async` + this crate together (Eric, 2026-09-21: after the lock-step round completes) | docs gates green ✅; publication un-holds at the release |
