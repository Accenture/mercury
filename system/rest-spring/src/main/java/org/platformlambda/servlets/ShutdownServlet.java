...
public class ShutdownServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (origin.equals(Platform.getInstance().getOrigin())) {
            event.setTo(PostOffice.SHUTDOWN_SERVICE);
        } else {
            if (!po.exists(origin)) {
                response.sendError(400, origin+" is not reachable");
                return;
            }
            event.setTo(PostOffice.SHUTDOWN_SERVICE+"@"+origin);
        }
        event.setHeader(TYPE, SHUTDOWN).setHeader(USER, System.getProperty("user.name"));
