...
- [ ] **Design + port event-script (layer 2)** — the working thread realizing
  ...
  **Next: E-3 — platform-core extensions** (event-interceptor registration mode, send_later/cancel,
  rest.yaml `flow:` → x-flow-id, body deep-copy) — a platform-core increment with its own tests.
  → serves: vision-mercury
