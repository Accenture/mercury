...
Also (same session, closure): **v4.10.2 SHIPPED** — tag `v4.10.2` on merge commit
`6a39bccc` (PR #172 incl. the config-race fix `0d09154d`, CI green), release published
2026-07-23, lock-step with the Java v4.10.2 (tag on `61ddb772`, PR #222 — same day);
[[thread-release-4-10-2]] marked `- [x]` (left in place for the review to sweep);
status line → 4.10.2. The metadata-hardening thread was already closed at prep time
(PR #171); no other thread was resolved by this release.
