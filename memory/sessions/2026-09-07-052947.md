...
**Post-merge close:** PR #238 merged 2026-09-07 as a true merge commit (`76b8f282` — all
four branch commits preserved, no squash-title concern this round); mains synced, branch
retired (remote auto-deleted), [[ot-rust-docs-sibling-sweep]] closed with its record.
Left for the next Java-repo round: refresh the Java continuity fact's "the Rust registry
lands with the held [[ot-rust-docs-sibling-sweep]]" sentence to LANDED (Rust PR #238,
ADR-0018) — the Java repo had no tracked change this session, so per protocol no Java
log was minted. Also owed here: the memory review (`[review-overdue]`, 12 ≥ 10).

## Memory Review (2026-09-07, through 2026-09-07-052947)
- Reactivated:   0
- Superseded:    0
- Archived:      0 facts
- Swept threads: 2 completed Open Threads → memory/archive/2026-Q3.md
  (ot-json-plugin-twin sslu 21, ot-route-pool-twin sslu 24 — both > archive_window 20;
  moved via archive-fact)
...
- Tier changes:  4 via refresh-metadata (playground-session-broker active→archive-candidate;
  conv-llms-txt-curated-map, why-ai-enabled-before-code, ot-rust-docs-sibling-sweep
  working→active) + claims-fixture-gate last_used refresh
- Invariants:    not due (19 sessions since the 2026-09-02 check < verify_invariants_every
  40; next due in ~21 sessions)
- Promoted core: 0 (auto-core off; core is human-set)
- Structural fixes: the three facts appended under `## Blueprint` on 2026-09-04 moved to
  their proper sections — conv-llms-txt-curated-map and
  conv-declare-consulted-references-rust → Conventions; why-ai-enabled-before-code →
  Key Decisions (footers untouched). Stale pointer inside inv-never-couple-functions
  refreshed ("ADR-0008 native" → "later entries native" — the ledger now runs to
  ADR-0018); wording only, substance untouched, NOT an invariant re-ratification.
- Contradiction scan: none. Altitude (VBDI) scan: no drift — the two (blueprint) threads
  serve vision-mercury; ot-cratesio-followups is housekeeping under the release rhythm.
- Smoke test: not re-run (full run 2026-09-04 alongside the last review; no
  memory-structure change since beyond this review's own moves).

## Memory References
- ot-rust-docs-sibling-sweep — executed and CLOSED (PR #238 merged)
- claims-fixture-gate — created (tier: working, tagged ADR-0018)
