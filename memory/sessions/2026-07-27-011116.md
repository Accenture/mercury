...
## Final update (same working session): v4.10.6 PUBLISHED

Eric confirmed publication (2026-07-26) — [[thread-release-4-10-6]] closed.
His thanks for the record: "Thank you for improving the Rust port."

Design validation worth keeping (Eric's field observation): the
async.http.response pretty-print improvement propagates to the MiniGraph
graph endpoint too — live proof: POST /api/graph/tutorial-2 on 8100 returns
pretty JSON. The single-render-path design means one presentation fix
reaches every JSON surface (functions, actuators, flows, graph executor)
