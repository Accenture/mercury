...
## Final update (same working session): P1 MERGED on both mains

Rust PR #181 (merge `ecee2df6`, CI 2m26s — the first trybuild CI run passed,
.stderr files matched stable first try) in lock-step with Java PR #234
(squash `265f295d`, CI 6m37s). Remaining in the arc = P2: D4
yaml.preload.override port + D5 registration-metadata contract page + golden
conformance fixture + ADR pair (trybuild, originally P2, already landed here).
