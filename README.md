...
cd rest-example
mvn clean package
java -Dcloud.connector=none -jar target/rest-example-1.12.57.jar
...
cd event-node
mvn clean package
java -jar target/event-node-1.12.57.jar
...
cd lambda-example
mvn clean package
java -Dcloud.connector=event.node -jar target/lambda-example-1.12.57.jar
...
java -Dcloud.connector=event.node -jar target/rest-example-1.12.57.jar
...
mvn clean package
java -jar target/hazelcast-presence-1.12.57.jar
...
java -Dcloud.connector=hazelcast -Dcloud.services=hazelcast.reporter -jar target/lambda-example-1.12.57.jar
...
java -Dcloud.connector=hazelcast -Dcloud.services=hazelcast.reporter -jar target/rest-example-1.12.57.jar
...
mvn clean package
java -jar target/kafka-standalone-1.12.57.jar
# this will start a standalone kafka server with embedded zookeeper
cd ../kafka-presence
mvn clean package
java -jar target/kafka-presence-1.12.57.jar
...
```bash
# go to the lambda-example project folder in one terminal
java -Dcloud.connector=kafka -Dcloud.services=kafka.reporter -jar target/lambda-example-1.12.57.jar
# the lambda-example will connect to the kafka server and the "presence monitor"

# go to the rest-example project folder in another terminal
java -Dcloud.connector=kafka -Dcloud.services=kafka.reporter -jar target/rest-example-1.12.57.jar
...
```json
{
    "apps":[
    {
        ...
        "args":[
            "-jar",
            "/full_qualified_path/event-node-1.12.57.jar"
        ],
        "watch":[
            "/full_qualified_path/event-node-1.12.57.jar"
        ],
