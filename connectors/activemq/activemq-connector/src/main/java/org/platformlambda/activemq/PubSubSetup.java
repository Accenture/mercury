...
public class PubSubSetup implements CloudSetup {
    ...
    public void initialize() {
        if (!PubSub.getInstance().featureEnabled()) {
            try {
                PubSub.getInstance().enableFeature(new ActiveMqPubSub());
                log.info("ActiveMQ cluster = {}",  ActiveMqSetup.getDisplayUrl());
            } catch (JMSException e) {
                log.error("Unable to connect to ActiveMQ cluster {} - {}", ActiveMqSetup.getDisplayUrl(), e.getMessage());
                System.exit(10);
            }
        }
    }
