...
import org.platformlambda.node.MainApp;
...
public class ConfigManagerEndpoint {
    ...
    public Map<String, Object> getApps(@HeaderParam(X_API_KEY) String key)
        ...
        String configApiKey = ConfigManager.getApiKey();
        if (configApiKey == null) {
            throw new AppException(503, "Config management service not ready ");
        }
        if (key == null) {
    ...
    public Object saveConfig(@PathParam(APPLICATION) String app,
        ...
        PostOffice po = PostOffice.getInstance();
        EventEnvelope res = po.request(MainApp.CONFIG_MANAGER, 5000,
                kv, new Kv(TYPE, UPDATE), new Kv(APPLICATION, app), new Kv(ENTITY, entity));
        return res.getBody();
    }
    ...
    public Object delConfig(@PathParam(APPLICATION) String app,
        ...
        PostOffice po = PostOffice.getInstance();
        EventEnvelope res = po.request(MainApp.CONFIG_MANAGER, 5000,
                kv, new Kv(TYPE, DELETE), new Kv(APPLICATION, app), new Kv(ENTITY, entity));
        return res.getBody();
    }
